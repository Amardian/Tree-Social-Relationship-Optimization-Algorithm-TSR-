...
for n=1:2:pn
    ...
    i1=randi(pn,1);
    i2=randi(pn,1);
    [prolifpop(2*pn+n).x,prolifpop(2*pn+n+1).x]=SinglePointProlifex(popk(i1).x,popk(i2).x);
    
    prolifpop(2*pn+n)  =fitness(prolifpop(2*pn+n),data);
    prolifpop(2*pn+n+1)=fitness(prolifpop(2*pn+n+1),data);
    
    prolifpop(2*pn+n).SDJ=0;
    prolifpop(2*pn+n+1).SDJ=0;
    
    prolifpop(2*pn+n).GR=(prolifpop(2*pn+n).fit/((popk(i1).fit+popk(i2).fit)/2))*100;
    prolifpop(2*pn+n+1).GR=(prolifpop(2*pn+n+1).fit/((popk(i1).fit+popk(i2).fit)/2))*100;
end
end
...
function [y1,y2]=SinglePointProlifex(x1,x2)
