function  Germinationpop = Germination(Germinationpop,data)


gn = data.gn+2;
n = data.n;
for i=1:gn
    Germinationpop(gn+i).x = swap(Germinationpop(i).x);
    Germinationpop(gn+i)=fitness(Germinationpop(gn+i),data);
    Germinationpop(gn+i)=weightcom(Germinationpop(gn+i),data);
    Germinationpop(gn+i).SDJ = 0;
    Germinationpop(gn+i).GR = (Germinationpop(gn+i).fit/(Germinationpop(i).fit))*100;
...
for i=1:gn
    ...
    Germinationpop(2*gn+i)=weightcom(Germinationpop(2*gn+i),data);
...
for i=1:gn
    ...
    Germinationpop(3*gn+i)=weightcom(Germinationpop(3*gn+i),data);
...
for i=1:gn
    ...
    Germinationpop(4*gn+i)=weightcom(Germinationpop(4*gn+i),data);
...
for i=1:gn
    ...
    Germinationpop(5*gn+i)=weightcom(Germinationpop(5*gn+i),data);
