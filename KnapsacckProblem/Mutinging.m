function mutepop= Mutinging(mutepop,data)
...
for n=1:mutn
    ...
    mutepop(mutn+n).x=Swap(mutepop(n).x);
    mutepop(mutn+n)=fitness(mutepop(mutn+n),data);
    mutepop(mutn+n)=weightcom(mutepop(mutn+n),data);
    mutepop(mutn+n).SDJ = 0;
    mutepop(mutn+n).GR = (mutepop(mutn+n).fit/mutepop(n).fit)*100;
...
for n=1:mutn
    ...
    mutepop(2*mutn+n)=weightcom(mutepop(2*mutn+n),data);
...
for n=1:mutn
    ...
    mutepop(3*mutn+n)=weightcom(mutepop(3*mutn+n),data);
